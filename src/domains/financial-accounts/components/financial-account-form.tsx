...
export function FinancialAccountForm({ protectedPersonId, account }: { protectedPersonId: string; account?: FinancialAccount }) {
  ...
  return <form action={formAction} className="grid gap-3 sm:grid-cols-2">
    ...
    <Field id="initialBalance" label="Solde initial" type="number" step="0.01" required defaultValue={account?.initial_balance ?? "0.00"} help="Indiquez la valeur réelle du compte à la date du solde initial. Ce montant sert de point de départ au suivi dans PatriGest ; 0,00 est proposé par défaut, mais n’est pas une valeur calculée." errors={state.fieldErrors?.initialBalance} /><Field id="initialBalanceDate" label="Date du solde initial" type="date" required defaultValue={account?.initial_balance_date} help="Choisissez la date réelle à laquelle correspond ce solde. Si le compte existait déjà avant son ajout dans PatriGest, indiquez la date historique du solde connu." errors={state.fieldErrors?.initialBalanceDate} />
