...
import { BankStatementManager } from "@/domains/bank-statements/components/bank-statement-manager";
...
export const metadata: Metadata = { title: "Relevés bancaires" };
...
export default async function Page({
  ...
  const reconciliations = await getBankReconciliationListStates(
    statementPage.items.map((item) => item.id),
  );
  ...
  const pathname = `/dossiers/${protectedPersonId}/comptes/${accountId}/releves`;
  ...
  return (
    ...
    >
      ...
      <h1 className="mt-1 text-2xl font-bold">Relevés bancaires</h1>
      ...
      <FinancialNavigation protectedPersonId={protectedPersonId} accountId={accountId} current="statements" />
