...
export default async function OperationsPage({ params, searchParams }: { params: Promise<{ protectedPersonId: string }>; searchParams: Search }) {
  ...
  const journalPage = corrective ? null : await getTransactionJournalPage(protectedPersonId, { startDate, endDate, accountId: filterValues.account, type: filterValues.type, categoryId: filterValues.category, query: filterValues.q }, requestedPage);
  const items = corrective ? corrective.items : journalPage!.items;
  const correctiveQuery = corrective ? `reportId=${corrective.report.id}&classificationIssue=${requestedIssue}` : null;
  const pathname = `/dossiers/${protectedPersonId}/operations`;
  const journalHref = journalPage ? getTransactionJournalPageHref(pathname, filterValues, journalPage.page) : pathname;
  const returnTo = getSafeTransactionReturnTo(protectedPersonId, correctiveQuery ? `${pathname}?${correctiveQuery}` : journalHref);
  return <PrivateShell current="dossiers" dossier={{ id: protectedPersonId, name: `${person.first_name} ${person.last_name}`, current: "operations" }}>
    ...
    {journalPage && <TransactionPagination pathname={pathname} values={filterValues} page={journalPage.page} totalPages={journalPage.totalPages} />}
