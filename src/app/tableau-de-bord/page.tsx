...
export default async function DashboardPage() {
  ...
  return <PrivateShell current="dashboard">
    ...
    <section className="mt-5 grid gap-3 sm:grid-cols-3" aria-label="Indicateurs principaux">
      <StatCard href="/dossiers" title="Dossiers actifs" value={data.activeDossierCount} icon={FolderOpen} color="blue" />
      <StatCard title="Comptes de gestion à traiter" value={data.reportActionCount} icon={ClipboardList} color="amber" />
...
function DossiersSection({ dossiers, canCreate }: { dossiers: DashboardDossier[]; canCreate: boolean }) { return <section className="rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] sm:p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold">Dossiers suivis</h2><p className="text-xs text-[#64748B]">Personnes protégées dont vous pouvez consulter le dossier.</p></div><Link className="auth-link shrink-0 text-xs" href="/dossiers">Voir tous les dossiers</Link></div>{dossiers.length ? <div className="mt-3 grid items-start gap-3 md:grid-cols-2">{dossiers.map((dossier) => <DossierCard key={dossier.id} dossier={dossier} />)}</div> : <div className="mt-4 rounded-xl border border-dashed border-[#CBD5E1] p-5 text-center"><p className="text-sm font-semibold">Aucun dossier accessible actuellement.</p><p className="mt-1 text-xs text-[#64748B]">{canCreate ? "Vous pouvez créer votre premier dossier ou consulter la liste des dossiers." : "Consultez la liste des dossiers ou attendez qu’un accès vous soit accordé."}</p><div className="mt-3 flex flex-wrap justify-center gap-2">{canCreate && <Link className="button button-primary" href="/dossiers/nouveau">Créer mon premier dossier</Link>}<Link className="button button-secondary" href="/dossiers">Voir tous les dossiers</Link></div></div>}</section>; }

function DossierCard({ dossier }: { dossier: DashboardDossier }) {
  const destination = dossier.nextAction?.href ?? `/dossiers/${dossier.id}/tableau-de-bord`;
  return <article className="rounded-xl border border-[#E2E8F0] p-3.5"><div className="flex items-start gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-[#2563EB]"><UserRound aria-hidden="true" size={16} /></span><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-bold">{dossier.firstName} {dossier.lastName}</h3><p className="text-[11px] font-semibold text-[#64748B]">{roleLabels[dossier.accessRole]} · {dossier.activeAccountCount} {dossier.activeAccountCount > 1 ? "comptes actifs" : "compte actif"}</p></div></div><div className="mt-3 border-t border-[#E2E8F0] pt-2.5"><ReportSummary report={dossier.latestReport} />{dossier.nextAction && <p className="mt-1.5 line-clamp-2 text-[11px] text-[#64748B]"><span className="font-semibold text-[#334155]">Prochaine action :</span> {dossier.nextAction.label}</p>}</div><Link className="focus-ring mt-3 inline-flex items-center gap-1 rounded text-xs font-bold text-[#2563EB] hover:text-blue-700" href={destination}>{dossier.nextAction ? "Traiter la prochaine action" : "Ouvrir le dossier"} <ArrowRight aria-hidden="true" size={14} /></Link></article>;
}
...
function TasksSection({ tasks }: { tasks: DashboardTask[] }) {
  const actionable = tasks.filter((task) => task.actionable);
  const information = tasks.filter((task) => !task.actionable);
  const content = (task: DashboardTask) => <><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs font-bold text-[#334155]">{task.personName}</p><p className="mt-0.5 text-sm font-semibold">{task.label}</p><p className="mt-0.5 text-[11px] text-[#64748B]">Du {formatFinancialDate(task.startDate)} au {formatFinancialDate(task.dueDate)}</p></div>{task.actionable && <ArrowRight aria-hidden="true" className="mt-1 shrink-0 text-[#EA580C]" size={15} />}</div><p className="mt-1 text-[11px] font-semibold text-[#9A3412]">Échéance : {formatFinancialDate(task.dueDate)}</p></>;
  return <section className="rounded-2xl border border-amber-100 bg-amber-50/60 p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)] sm:p-5"><div className="flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-lg bg-amber-100 text-[#EA580C]"><CalendarDays aria-hidden="true" size={18} /></span><div><h2 className="text-lg font-bold">À faire prochainement</h2><p className="text-xs text-[#64748B]">Actions classées par priorité et échéance.</p></div></div>{actionable.length ? <div className="mt-3 divide-y divide-amber-100">{actionable.map((task) => <Link key={task.id} href={task.href} className="focus-ring -mx-2 block rounded-lg px-2 py-3 transition hover:bg-white/70">{content(task)}</Link>)}</div> : <p className="mt-4 text-sm font-semibold text-[#64748B]">Aucune action à traiter actuellement.</p>}{information.length > 0 && <div className="mt-4 border-t border-amber-100 pt-3"><h3 className="text-sm font-semibold text-[#64748B]">Pour information</h3><div className="mt-1 divide-y divide-amber-100">{information.map((task) => <article key={task.id} className="py-2">{content(task)}</article>)}</div></div>}</section>;
}
